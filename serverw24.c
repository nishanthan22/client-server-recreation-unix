...
#define MIRROR_1_PORT 4223
#define MIRROR_2_PORT 4224
#define HOST_ADDRESS "192.168.6.129"

#define CONN_TYPE_SERVER 22
#define CONN_TYPE_MIRROR_1 32
#define CONN_TYPE_MIRROR_2 42

int send_response(char * content, int type, int client_socket);
// Function to determine connection type based on client connection ID
int connection_type(int client_connection_id) {

  if (client_connection_id >= 1 && client_connection_id <= 3) {
    return CONN_TYPE_SERVER;
  } else if (client_connection_id >= 4 && client_connection_id <= 6) {
    return CONN_TYPE_MIRROR_1;
  } else if (client_connection_id >= 7 && client_connection_id <= 9) {
    return CONN_TYPE_MIRROR_2;
  } else {
    // Calculate the type based on the client_connection_id value
    int type = (client_connection_id - 10) % 3; // 0, 1, or 2
    if (type == 0)
      return CONN_TYPE_SERVER;
    else if (type == 1)
      return CONN_TYPE_MIRROR_1;
...
// Handler function for Mirror 1
int mirror1_handler(int client_socket, int connection_counter) {
  send_response("M1", RESPONSE_TEXT, client_socket);
  printf("Client number: %d\nThe output has redirected, please go to the mirror 1 terminal to see the output\n", connection_counter);
  return 0;
}
// Handler function for Mirror 2
int mirror2_handler(int client_socket, int connection_counter) {
  send_response("M2", RESPONSE_TEXT, client_socket);
  printf("Client number: %d\nThe output has redirected, please go to the mirror 2 terminal to see the output\n", connection_counter);
  return 0;
}
...
void remove_ldtr_spaces(char * str) {
...
char * get_root_operation(char * command) {
  ...
  str = strtok(command, " ");
  if (str != NULL) {
    remove_ldtr_spaces(str);
    return str;
  }

  return NULL;
}
// Function to extract the root operation from a command
...
// Function to find files of specified sizes in a directory
...
// Function to extract sizes from a command
        ...
        if (num_of_files > 0) {
          ...
          if (status == 0) {
            ...
            if (tr_status != 0) {
              ...
              send_response("File transfer successfully completed.", RESPONSE_TEXT, client_socket);
            }
          } else {
...
int main() {
  ...
  printf("Server is ready, waiting for the client to connect\n");
  // Move connection_counter outside the loop
  ...
  while (1) {
    ...
    connection_counter++;
    int SERVER_TYPE = connection_type(connection_counter);
    if (SERVER_TYPE == CONN_TYPE_SERVER) {
      ...
      printf("Oh no, too many load, requests will be redirected to respective mirrors.\n");

      // Now, child process will be forked for redirecting client to mirror.
      if (!fork()) {
        if (SERVER_TYPE == CONN_TYPE_MIRROR_1) {
          mirror1_handler(client_socket, connection_counter);
        } else if (SERVER_TYPE == CONN_TYPE_MIRROR_2) {
          mirror2_handler(client_socket, connection_counter);
        }
        close(client_socket);
        exit(0);
      }
